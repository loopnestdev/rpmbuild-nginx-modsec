...
##### ELYSIUM #####
Source10: clamd_scan.pl
Source11: custom.conf
Source12: main.conf
Source13: modsecurity.conf
Source14: unicode.mapping
Source15: echo-nginx-module-0.63.tar.gz
Source16: headers-more-nginx-module-0.38.tar.gz
Source17: nginx-module-vts-0.2.3.tar.gz
Source18: ModSecurity-nginx-1.0.3.tar.gz
Source19: ngx_http_geoip2_module-3.4.tar.gz
####################
...
./configure %{BASE_CONFIGURE_ARGS} \
    ...
    --add-dynamic-module="%{_builddir}/%{name}-%{version}/echo-nginx-module-0.63" \
    --add-dynamic-module="%{_builddir}/%{name}-%{version}/headers-more-nginx-module-0.38" \
    --add-dynamic-module="%{_builddir}/%{name}-%{version}/nginx-module-vts-0.2.3" \
    --add-dynamic-module="%{_builddir}/%{name}-%{version}/ModSecurity-nginx-1.0.3" --with-compat \
    --add-dynamic-module="%{_builddir}/%{name}-%{version}/ngx_http_geoip2_module-3.4" --with-compat \
    --with-debug
make %{?_smp_mflags}
...
%{__mkdir} -p $RPM_BUILD_ROOT%{_sysconfdir}/nginx/apm.d
%{__mkdir} -p $RPM_BUILD_ROOT%{_sysconfdir}/nginx/ssl
%{__mkdir} -p $RPM_BUILD_ROOT%{_sysconfdir}/nginx/modsec
%{__mkdir} -p $RPM_BUILD_ROOT%{_sysconfdir}/nginx/owasp
%{__mkdir} -p $RPM_BUILD_ROOT%{_sysconfdir}/nginx/geoip
###################
...
##### ELYSIUM #####
%{__install} -m755 -p %{SOURCE10} \
    $RPM_BUILD_ROOT%{_prefix}/local/bin/clamd_scan.pl
%{__install} -m644 -p %{SOURCE11} \
    $RPM_BUILD_ROOT%{_sysconfdir}/nginx/conf.d/custom.conf
%{__install} -m644 -p %{SOURCE12} \
    $RPM_BUILD_ROOT%{_sysconfdir}/nginx/modsec/main.conf
%{__install} -m644 -p %{SOURCE13} \
    $RPM_BUILD_ROOT%{_sysconfdir}/nginx/modsec/modsecurity.conf
%{__install} -m644 -p %{SOURCE14} \
    $RPM_BUILD_ROOT%{_sysconfdir}/nginx/modsec/unicode.mapping
#################
...
##### ELYSIUM #####
%dir %{_sysconfdir}/nginx/apm.d
%dir %{_sysconfdir}/nginx/ssl
%dir %{_sysconfdir}/nginx/modsec
%dir %{_sysconfdir}/nginx/owasp
%dir %{_sysconfdir}/nginx/geoip
###################
...
##### ELYSIUM #####
%{_prefix}/local/bin/clamd_scan.pl
%config(noreplace) %{_sysconfdir}/nginx/conf.d/custom.conf
...
##### ELYSIUM #####
Elysium Additional Components for this NGINX:
  - ModSecurity: 3.0.13 (https://github.com/SpiderLabs/ModSecurity/releases/tag/v3.0.13)
  - ModSecurity-Nginx Connector 1.0.3 (https://github.com/SpiderLabs/ModSecurity-nginx/releases/tag/v1.0.3)
  - headers-more-nginx-module: v0.38 (https://github.com/openresty/headers-more-nginx-module/releases/tag/v0.38)
  - echo-nginx-module: v0.63 (https://github.com/openresty/echo-nginx-module/releases/tag/v0.63)
  - nginx-module-vts: 0.2.3 (https://github.com/vozlt/nginx-module-vts/releases/tag/v0.2.3)
  - nginx-http-geoip2-module: 3.4 (https://github.com/leev/ngx_http_geoip2_module/tree/3.4)
  - Lua: 5.4
###################
----------------------------------------------------------------------
BANNER
...
%changelog
* Wed Feb  5 2025 Nginx Packaging <nginx-packaging@f5.com> - 1.26.3-1%{?dist}.ngx
- 1.26.3-1

* Wed Nov 20 2024 Nginx Packaging <nginx-packaging@f5.com> - 1.26.2-2%{?dist}.ngx
- RHEL 9 only: bumped Epoch to override appstream version.

* Wed Aug 14 2024 Nginx Packaging <nginx-packaging@f5.com> - 1.26.2-1%{?dist}.ngx
- 1.26.2-1
